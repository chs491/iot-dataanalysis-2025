...
- 강화 학습(reinforment learn) - 선택가능한 행동 중 보상과 처벌 등으로 최적의 행동양식을 학습하는 것 

#### 훈련 세트 / 테스트 세트
- 훈련 세트 - 모델을 훈련시키기 위한 데이터
- 테스트 세트 - 훈련 후 모델이 에측을 제대로 하는지 테스트하는 데이터

- 전체 데이터 70~80 분리 후 훈련세트로, 20~30퍼센트를 테스트 세트 사용
...
- 위 문제를 해결하기 위해서 랜덤하게 섞어 줌    

#### 넘파이
- 수학 라이브러리 일종. 파이썬에서 배열처리 쉽게 도와주기 위해 개발
- 2차원 배열이상 고차원 배열 조각 처리 간편한 도구
...
- `과소적합` - underfit. 모델 훈련세트 성능이 낮거나, 테스트세트 성능이 너무 높을때    
...
#### 특성공학
- 훈련시킬 특성이 모자랄 때 기존의 특성을 조합해서 새로운 특성을 만드는 과정
...
- 활성화함수 - 입력신호를 출력신호로 변환시켜주는 함수
    - `시그모이드 함수` - z가 아주 큰 음수일때 0으로, z가 아주 큰 양수일때 1로 바꿔주는 함수      
